...
/*   Updated: 2018/02/19 14:54:34 by bjanik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	num_is_eligible_to_be_an_entry(char *num, t_history *history)
{
	int	position;

	(num[0] == '+') ? num++ : 0;
	if (ft_strisdigit(num))
	{
		if (ft_strlen(num) > ft_strlen(INT_MAX_STR)
				|| ft_strcmp(num, INT_MAX_STR) >= 0)
			return (0);
	}
	else
...
static int	history_del_one_entry(t_history *history, const char *num)
{
	if (num)
	{
		if (num_is_eligible_to_be_an_entry(num) == 1)
			delete_cmd_from_history(history, ft_atoi(num));
		else
...
int	builtin_history(char **args, t_env *env)
{
	...
	while (args[++i] && args[i][0] == '-' && ft_strcmp("--", args[i]))
	{
		if (!args[i][1])
		{
			ft_putendl_fd("bsh: history: `-': not a valid identifier",
					STDERR_FILENO);
				return (1);
		}
