...
/*   Updated: 2018/03/02 16:49:08 by heynard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						print_grammar_source(t_grammar const *grammar,
		int fd_source_file,
		char const *no_use);
void						print_grammar_proto(t_grammar const *grammar,
		int fd_proto_file,
		char const *name);
void						print_grammar_init(t_grammar const *grammar,
		int fd_init_file,
		char const *header_name);
void						print_grammar_header(t_grammar const *grammar,
		int fd_header_file,
		char const *name);
...
t_bool						is_of_class(t_grammar const *gram,
						t_gram_class type);
...
t_bool						gram_are_equ(t_grammar const *gram_1,
							t_grammar const *gram_2);
...
t_symbol					*find_sym_by_name(t_grammar const *gram,
					char const *sym_name);
...
t_lst						*str_to_first_set_lst(char const **array,
		size_t nb_sym);
